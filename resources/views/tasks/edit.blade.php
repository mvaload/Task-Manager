                    ...
                    <form method="POST" action="{{ route('tasks.update', $task->id) }}">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select id="tags" class="form-control @error('tags') is-invalid @enderror" name="tags">
                                    <option>
                                        @foreach ($tags as $tag)
                                    <option value="{{ $tag->id }}" {{ $task->id == $tag->id ? ' selected' : '' }}>{{ $tag->name }}</option>
                                    @endforeach
                                </select>
                                @error('tags')
                                <span class="invalid-feedback" role="alert">
                        ...
                        <div class="form-group row mb-0">
                            <div class="col-md-6 offset-md-4">
                                ...
                                <a href="{{ route('tasks.destroy', $task->id) }}" class="btn btn-danger" data-method="delete" data-confirm="Are you sure you want to delete this task?">
                                    {{ __('Delete') }}
                                </a>
                            </div>
                        </div>
